...
<table  class="table table-striped tableWidth"> \
...
<%}%>',
        propertyattribute:
'<%if (properties.length > 0 || attributes.length > 0) {%><table class="table table-hover tableWidth"> \
    <% for (var p in properties) { var prop = properties[p];%> \
    <tr> \
        <td><%=prop.name%></td>\
        <td><%=prop.value%></td>\
...
<%}%>',
        entity: '<%= name? name: (function f() { return type.charAt(0).toUpperCase() + type.slice(1); })() %>',
        contact:
